...
function renderDevices() {
  ...
  if (selfInfo) {
    ...
    var t = document.createTextNode(meName + (meHost ? ' · host' : '') + ' · you');
    selfInfo.appendChild(t);
    if (!meHost) {
      var a = document.createElement('a');
      a.href = '#';
      a.textContent = 'Make me host';
      a.style.marginLeft = '6px';
      a.style.color = '#fff';
      a.style.fontSize = '11px';
      a.style.textDecoration = 'underline';
      a.onclick = function(ev){ ev.preventDefault(); amIHost=true; try{localStorage.setItem('ftIsHost','1');}catch(e){} registerDevice(); };
      selfInfo.appendChild(document.createTextNode(' '));
      selfInfo.appendChild(a);
    } else if (amIHost && me && !me.isHost) {
      // server hasn't synced yet — re-register as host
      amIHost=true; try{localStorage.setItem('ftIsHost','1');}catch(e){}
    }
  }
